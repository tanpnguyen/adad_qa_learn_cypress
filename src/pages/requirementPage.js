import requirementSelectors from '../selectors/requirementPageSelectors.json'
...
class requirementPage extends basePage {
    ...
    createNewRequirement = (projectName, moduleId, requirementName, priority, description) => {
        ...
        cy.get(requirementSelectors.btnNewRequirement)
        .click({force: true})

        // Validate title of Requirement page
        cy.get(requirementSelectors.titleRequirement).then(($span) => {
            cy.wrap($span).should('be.visible')
            .should('have.text', 'Requirement')
        })
                
        //enter Requirement name 
        cy.get(requirementSelectors.txtRequirementName)
        .should('be.visible')
        .dblclick()
        .type('{del}', {delay: 2000})
        .type(requirementName,{delay: 30})
        .type('{enter}')
